...
public class View extends JFrame {
	...
	public void init() {
		...
		typeGraphPanel.setBorder(
				BorderFactory.createTitledBorder(redBordered, "Loại Đồ Thị", TitledBorder.CENTER, TitledBorder.TOP,
						new Font("Tahoma", Font.BOLD, 13), Color.RED));
		...
		boolean[] isDirGraph = { false }; // Khởi tạo mảng chứa giá trị của biến cờ

		JPanel algoPrimPanel = new JPanel(new GridBagLayout());
		algoPrimPanel.setBorder(
				BorderFactory.createTitledBorder(redBordered, "Tìm cây khung nhỏ nhất", TitledBorder.CENTER,
		...
		checkConnectedPanel.setBorder(
				BorderFactory.createTitledBorder(redBordered, "Kiểm tra tính liên thông", TitledBorder.CENTER,
						TitledBorder.TOP, new Font("Tahoma", Font.BOLD, 13), Color.RED));
		btnFindMST.addActionListener(e -> {

			// if (isDirGraph[0]) { // Kiểm tra xem người dùng đã chọn đồ thị có hướng hay
			// chưa
			// JOptionPane.showMessageDialog(null, "Thuật toán Prim chỉ áp dụng cho đồ thị
			// vô hướng.");
			// return;
			// }

			algo = new Algo(matrixWeight);
			int numConnectedComponents = algo.countConnectedComponents(matrixWeight); // Đếm số lượng thành phần liên
																						// thông
			if (numConnectedComponents != 1) { // Nếu số lượng thành phần liên thông lớn hơn 1, thông báo cho người dùng
												// và không chạy thuật toán Prim
				JOptionPane.showMessageDialog(null, "Đồ thị không liên thông!");
				return;
			}
			String findMST = algo.showMST(matrixWeight, rdbUnGraph.isSelected());
			resultTextArea.setText(findMST);
			matrixTextArea.setText(algo.matrixToString(algo.getMSTMatrix(matrixWeight)));
		});

		JButton btnConnectedComponents = new JButton("Kiểm tra liên thông");
		btnConnectedComponents.addActionListener(e -> {
			algo = new Algo(matrixWeight);
			int count = algo.countConnectedComponents(matrixWeight);
			String result = "";
			...
			matrixTextArea.setText(algo.matrixToString(matrixWeight));
		});
		...
		drawPanel.addComponentListener(new ComponentAdapter() {
			...
			public void componentResized(ComponentEvent e) {
				try {
					...
					// int firstVex = listdraw.get(listdraw.size() - 2).p;
					// int endVex = listdraw.get(listdraw.size() - 1).p;
					// int weight = Integer.parseInt(weightTextField.getText());
					// arrWeight[firstVex - 1][endVex - 1] = weight;
		...
		functionPanel.setBorder(
				BorderFactory.createTitledBorder(redBordered, "Chức năng", TitledBorder.CENTER, TitledBorder.TOP,
						new Font("Tahoma", Font.BOLD, 13), Color.RED));
		...
		btnDel.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (!dp1.isEmpty() && !dp2.isEmpty()) {
					firstVex = Integer.parseInt(dp1);
					endVex = Integer.parseInt(dp2);
				}
				if (e.getActionCommand().equals("Thay đổi cạnh")) {
					if (dp1.isEmpty() || dp2.isEmpty()) {
						JOptionPane.showMessageDialog(View.this, "Không có điểm đầu hoặc điểm cuối", "Lỗi",
								JOptionPane.ERROR_MESSAGE);
					} else {
						String message = "Bạn có muốn sửa cạnh từ " + firstVex + " đến " + endVex + " ?";
						int select = JOptionPane.showConfirmDialog(View.this, message, "Sửa cạnh",
								JOptionPane.OK_CANCEL_OPTION);
						if (select == JOptionPane.OK_OPTION) {

							String newWeightString = JOptionPane.showInputDialog(View.this,
									"Nhập trọng số mới cho cạnh từ " + firstVex + " đến " + endVex + ": ");
							// ...
							double newWeight = Double.parseDouble(newWeightString);
							if (newWeightString != null && !newWeightString.isEmpty()) {
								try {
									if (rdbUnGraph.isSelected() == true) {

										matrixWeight[firstVex - 1][endVex - 1] = (int) newWeight;
										matrixWeight[endVex - 1][firstVex - 1] = (int) newWeight;
										Graphics g = getGraphics();
										for (int i = 0; i < listVertex.size(); i++) {
											DrawGraph p1 = listVertex.get(i);
											for (int j = i + 1; j < listVertex.size(); j++) {
												...
												if (w > 0) {
													if (rdbDirGraph.isSelected()) {
														...
														g.fillRect((p1.x + p2.x + 10) / 2, (p1.y + p2.y) / 2 + 116, 100,
																100);
														g.drawString(Integer.toString(w), (p1.x + p2.x + 10) / 2,
																(p1.y + p2.y) / 2 + 116);
														g.setColor(Color.black);
														p1.drawArrowLine(g, p1.x + 11, p1.y + 100, p2.x + 12,
																p2.y + 100, 5, 5);
													} else {
														// g.setColor(Color.red);
														g.drawString(Integer.toString(w), (p1.x + p2.x + 10) / 2,
																(p1.y + p2.y) / 2 + 116);
														g.setColor(Color.black);
														p1.drawLineWithoutArrow(g, p1.x + 11, p1.y + 100, p2.x + 12,
																p2.y + 100, 0, 0);
													}
												}
											}
									...
									// TODO: Thực hiện sửa trọng số của cạnh từ firstVex đến endVex thành newWeight
								} catch (NumberFormatException ex) {
									JOptionPane.showMessageDialog(View.this, "Trọng số mới phải là một số thực", "Lỗi",
											JOptionPane.ERROR_MESSAGE);
								}
							}
						}
			...
			for (int i = 0; i < listVertex.size(); i++) {
				for (DrawGraph point : listVertex) {
					...
					for (int j = i + 1; j < listVertex.size(); j++) {
						...
						point1.eraseArrowLine(g, point1.x + 11, point1.y + 100, point2.x + 12,
								point2.y + 100, 5, 5);
		...
		JButton btnRe = new JButton("Thay đổi cạnh");
				...
				if (e.getActionCommand().equals("Thay đổi cạnh")) {
					if (dp1.isEmpty() || dp2.isEmpty()) {
						...
						if (select == JOptionPane.OK_OPTION) {
							...
							if (newWeightString != null && !newWeightString.isEmpty()) {
								try {
									double newWeight = Double.valueOf(newWeightString);
											...
											if (w > 0) {
												if (rdbDirGraph.isSelected()) {
													...
													g.drawString(Integer.toString(w), (p1.x + p2.x + 10) / 2,
															(p1.y + p2.y) / 2 + 116);
													g.setColor(Color.black);
													p1.drawArrowLine(g, p1.x + 11, p1.y + 100, p2.x + 12, p2.y + 100, 5,
															5);
												} else {
													g.setColor(Color.red);
													g.drawString(Integer.toString(w), (p1.x + p2.x + 10) / 2,
									...
									JOptionPane.showMessageDialog(View.this, "Trọng số không hợp lệ", "Lỗi",
											JOptionPane.ERROR_MESSAGE);
								}
							}
						}
		...
		btnReadFile.addActionListener(e -> {
			...
			if (userSelection == JFileChooser.APPROVE_OPTION) {
				...
				// Algo algo = new Algo(selectedFile);
				// algo.loadFile();
		...
		// mainButtonPanel.add(btnReadFile);
		...
		resultPanel.setBorder(
				BorderFactory.createTitledBorder(redBordered, "Hiển thị", TitledBorder.CENTER, TitledBorder.TOP,
						new Font("Tahoma", Font.BOLD, 14), Color.RED));
